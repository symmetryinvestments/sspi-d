...
enum GPO_SECTION_ROOT                 = 0;  // Root
enum GPO_SECTION_USER                 = 1;  // User
enum GPO_SECTION_MACHINE              = 2;  // Machine
...
alias PGROUP_POLICY_OBJECT_TYPE= PGROUP_POLICY_OBJECT_TYPE*;
...
alias PGROUP_POLICY_HINT_TYPE= PGROUP_POLICY_HINT_TYPE*;
...
alias LPGPEINFORMATION = IGPEInformation*;
...
enum GPO_OPEN_LOAD_REGISTRY      = 0x00000001;  // Load the registry files
enum GPO_OPEN_READ_ONLY       = 0x00000002;  // Open the GPO as read only
...
enum GPO_OPTION_DISABLE_USER     = 0x00000001;  // The user portion of this GPO is disabled
enum GPO_OPTION_DISABLE_MACHINE = 0x00000002;  // The machine portion of this GPO is disabled
...
DECLARE_INTERFACE_(IGroupPolicyObject, IUnknown)
{
    ...
    STDMETHOD(New) (THIS_ LPOLESTR pszDomainName, _In_opt_ LPOLESTR pszDisplayName,
                    DWORD dwFlags) PURE;
...
enum RSOP_INFO_FLAG_DIAGNOSTIC_MODE  = 0x00000001;      // Running in diagnostic mode vs planning mode
...
DECLARE_INTERFACE_(IRSOPInformation, IUnknown)
{
    ...
    STDMETHOD(GetEventLogEntryText) (THIS_ LPOLESTR pszEventSource, LPOLESTR pszEventLogName,
                                           LPOLESTR pszEventTime, DWORD dwEventID,  _Outptr_ LPOLESTR *ppszText) PURE;

};
alias LPRSOPINFORMATION = IRSOPInformation*;
...
enum GPO_BROWSE_DISABLENEW           = 0x00000001;  // Disables the New GPO functionality on all pages except "All"
enum GPO_BROWSE_NOCOMPUTERS          = 0x00000002;   // Removes the Computers tab
enum GPO_BROWSE_NODSGPOS             = 0x00000004;   // Removes the Domain/OU and Sites tabs
enum GPO_BROWSE_OPENBUTTON           = 0x00000008;   // Change the Ok button to say Open
enum GPO_BROWSE_INITTOALL            = 0x00000010;   // Initialize the dialog focused on the All pane
enum GPO_BROWSE_NOUSERGPOS           = 0x00000020;   // Removes the Users tab
enum GPO_BROWSE_SENDAPPLYONEDIT      = 0x00000040;   // Send the apply event if edit menu is selected
...
alias LPGPOBROWSEINFO= GPOBROWSEINFO*;
